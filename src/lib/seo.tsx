...
export function jsonLdScripts(opts: JsonLdOpts) {
  ...
  const website = {
    ...
    name: SITE_NAME,
    url: `${SITE_ORIGIN}/`,
    description: "Compress JPG, PNG and WebP in the browser with Canvas. No upload, no signup, no watermark.",
    inLanguage: "en",
    publisher,
  };
  const breadcrumb = {
...
function withCanonical(
  ...
  description: string,
  path: string,
  extra?: { appName: string; faqs?: readonly FaqItem[]; howToName?: string; howToSteps?: string[]; includeApp?: boolean },
) {
  const url = absUrl(path);
  const fullTitle = title.includes(" | Crush") || title.startsWith("Crush") ? title : `${title} | Crush`;
  return {
    meta: [
      { title: fullTitle },
