...
function savings(original: number, next: number) {
...
export function CompressorApp() {
  ...
  const addFiles = useCallback((list: FileList | File[]) => {
    ...
    Array.from(list).forEach((file) => {
      ...
      if (file.size > WARN_FILE_BYTES) {
        warnings.push(`${file.name} is large (${formatBytes(file.size)}). Compression may be slow on this device.`);
      }
      next.push({
  ...
  async function downloadZip() {
    ...
    await downloadBlob(blob, `crush-${files.length}-images.zip`);
  }
  ...
  const totals = useMemo(() => {
    ...
    return { count: done.length, original, next, saved: original - next };
  }, [items]);
  ...
  const savedPct = totals.original > 0 ? savings(totals.original, totals.next) : 0;
      ...
      <section className="mt-6 rounded-2xl border border-line bg-surface p-5">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            />
            <p className="mt-1 text-xs text-muted">PNG ignores quality (lossless). JPG/WebP use this slider.</p>
          </div>
          <div>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          ...
          >
            {stage === "working" ? (
              ...
              `Compress${items.length ? ` ${items.length}` : ""}`
            )}
          </Button>
                ...
                {item.error ? (
                  ...
                  <p className="text-sm tabular-nums text-success">
                    ...
                    <span className="ml-2 inline-flex rounded-full bg-paper px-2 py-0.5 text-xs font-semibold text-success">
                      {savings(item.result.originalSize, item.result.newSize) >= 0
                        ? `−${savings(item.result.originalSize, item.result.newSize)}%`
                        : `+${Math.abs(savings(item.result.originalSize, item.result.newSize))}%`}
                    </span>
                  </p>
                ) : (
      ...
      {stage === "done" && totals.count > 0 && (
        <section className="mt-6 rounded-2xl border border-line bg-paper p-5">
          <p className="font-display text-xl">
            Saved {formatBytes(Math.max(0, totals.saved))}
            {savedPct > 0 ? ` (${savedPct}% smaller)` : ""} across {totals.count}{" "}
          ...
          <p className="mt-3 text-xs text-muted">Output as {outputExt(outputType).toUpperCase()}. No watermark.</p>
